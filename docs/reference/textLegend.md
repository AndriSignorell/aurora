...
collisions are resolved via [`spreadOut()`](spreadOut.md). Callers
therefore simply pass the values in series order, parallel to `col`,
`lty` and `lwd`.
...
coordinates via [`lineToUser()`](lineToUser.md).
